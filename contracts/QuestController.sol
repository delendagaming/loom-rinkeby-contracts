...
contract QuestController {
    ...
    modifier onlyOwner {
        require(
            msg.sender == owner, "Only the contract owner can use this function"        );
        _;
    } 
    ...
    event paymentForQuest(address payer, address recipient, string identifierForQuest);
    event victory(address player, string questName, uint tokenID);
    ...
    function addEntry ( string memory questName,  address  creatorInput,  string  memory identifierInput, address verifierInput, uint priceInput) public onlyOwner {
        ...
        questRegister[questNameHash] =  QuestEntry(creatorInput, identifierInput, verifierInput, priceInput);
    ...
    function playerPayment (string memory questName) public {
        ...
        require(questRegister[questNameHash].creator != address(0), "This quest doesn't exist");
        ...
        require(bytes(_playerQuestInProgress[msg.sender]).length == 0, "You are already playing another quest");
    ...
    function checkAndMint(string memory questName, uint[2] memory a,
                  uint[2] memory a_p,
                  uint[2][2] memory b,
                  uint[2] memory b_p,
                  uint[2] memory c,
                  ...
                  uint[1] memory input) public {
                    ...
                    questNameHash = sha256(abi.encodePacked(questName));
                      ...
                      require(sha256(abi.encodePacked(_playerQuestInProgress[msg.sender])) == sha256(abi.encodePacked(questName)), "You are not currently playing this quest.");
                      
                      bool confirmation;
                      confirmation = _checkSolution(questName, a, a_p, b, b_p, c, c_p, h, k, input);
    ...
    function _checkSolution (string memory questName, uint[2] memory a,
                  uint[2] memory a_p,
                  uint[2][2] memory b,
                  uint[2] memory b_p,
                  uint[2] memory c,
                  ...
                  uint[2] memory h,
                  uint[2] memory k,
                  uint[1] memory input) private view returns (bool) {
    ...
    function abandonCurrentQuest() public {
        require(bytes(_playerQuestInProgress[msg.sender]).length > 0, "You are not playing any quest currently");
        _playerQuestInProgress[msg.sender] = '';
    }
