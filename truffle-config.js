  ...
  networks: {
    ...
    extdev_plasma_us1: {
      provider: function() {
        ...
        const mnemonicPath = path.join(__dirname, "extdev_mnemonic_CREATOR");
        const privateKeyPath = path.join(
          __dirname,
          "extdev_private_key_CREATOR"
        );
        if (fs.existsSync(privateKeyPath)) {
          const loomTruffleProvider = getLoomProviderWithPrivateKey(
            privateKeyPath,
    ...
    rinkeby: {
      provider: function() {
        ...
        const mnemonicPath = path.join(__dirname, "rinkeby_mnemonic_CREATOR");
        const privateKeyPath = path.join(
          __dirname,
          "rinkeby_private_key_CREATOR"
        );
        if (fs.existsSync(privateKeyPath)) {
          ...
          const mnemonic = readFileSync(
            path.join(__dirname, "rinkeby_mnemonic_CREATOR"),
            "utf-8"
          );
