...
const QuestLootRinkebyJSON = require("./src/contracts/QuestLootRinkeby.json");
const QuestCoinRinkebyJSON = require("./src/contracts/QuestCoinRinkeby.json");
const QuestLootLoomJSON = require("./src/contracts/QuestLootLoom.json");
const QuestCoinLoomJSON = require("./src/contracts/QuestCoinLoom.json");
...
async function getRinkebyCoinContract(web3js) {
  ...
  return new web3js.eth.Contract(
    QuestCoinRinkebyJSON.abi,
    QuestCoinRinkebyJSON.networks[networkId].address
  );
}

async function getRinkebyCoinContractAddress(web3js) {
  const networkId = await web3js.eth.net.getId();
  return QuestCoinRinkebyJSON.networks[networkId].address;
}
...
async function getRinkebyTokenContract(web3js) {
  const networkId = await web3js.eth.net.getId();
  return new web3js.eth.Contract(
    QuestLootRinkebyJSON.abi,
    QuestLootRinkebyJSON.networks[networkId].address
  );
}
...
async function getRinkebyTokenBalance(web3js, accountAddress) {
  ...
  const total = await contract.methods.balanceOf(accountAddress).call();
  const tokens = [];
...
async function getExtdevCoinContract(web3js) {
  const networkId = await web3js.eth.net.getId();
  return new web3js.eth.Contract(
    QuestCoinLoomJSON.abi,
    QuestCoinLoomJSON.networks[networkId].address
  );
}
...
async function getExtdevTokenContract(web3js) {
  const networkId = await web3js.eth.net.getId();
  return new web3js.eth.Contract(
    QuestLootLoomJSON.abi,
    QuestLootLoomJSON.networks[networkId].address
  );
}
...
async function depositCoinToExtdevGateway({
  ...
  try {
    ...
    console.error(
      "Withdraw failed while trying to approve token transfer to DAppChain Gateway."
    );
    throw err;
  }
    ...
    const listener = event => {
      ...
      if (
        ...
        gatewayContract.removeAllListeners(
          TransferGateway.EVENT_TOKEN_WITHDRAWAL
        );
        resolve(event);
      }
    };
  ...
  const tokenExtdevAddr = Address.fromString(
    `${client.chainId}:${tokenExtdevAddress}`
  );
  ...
  try {
    ...
    console.error(
      "Withdraw failed while trying to deposit tokens to DAppChain Gateway."
    );
    throw err;
  }
...
async function depositEthToExtdevGateway({
  ...
  const extdevGatewayAddr = Address.fromString(
    `${client.chainId}:${extdevGatewayAddress}`
  );
  ...
  await gatewayContract.withdrawETHAsync(
    new BN(amount),
    rinkebyGatewayAddr,
    ownerRinkebyAddr
  );
...
function generatePrivateKeyFromSeed(mnemonic) {
...
function loadRinkebyAccount() {
  /*
  const rinkebySeed = fs.readFileSync(
    path.join(__dirname, "./rinkeby_mnemonic"),
    "utf-8"
  );
  const privateKey = generatePrivateKeyFromSeed(rinkebySeed);
*/
...
async function mapAccounts({
  ...
  try {
    ...
    console.log(
      `${mapping.from.toString()} is already mapped to ${mapping.to.toString()}`
    );
    return;
  } catch (err) {
  ...
  console.log(
    `mapping ${ownerRinkebyAddr.toString()} to ${ownerExtdevAddr.toString()}`
  );
...
program
  ...
  .description(
    "deposit the specified amount of ERC20 tokens into the Transfer Gateway"
  )
...
program
  ...
  .description(
    "withdraw the specified amount of ERC20 tokens via the Transfer Gateway"
  )
  .option("-g, --gas <number>", "Gas for the tx")
  .option(
    "--timeout <number>",
  ...
  .action(async function(amount, options) {
    ...
    try {
      ...
      const receipt = await depositCoinToExtdevGateway({
        ...
        tokenExtdevAddress: QuestCoinLoomJSON.networks[extdevNetworkId].address,
        tokenRinkebyAddress:
          QuestCoinRinkebyJSON.networks[rinkebyNetworkId].address,
...
program
  ...
  .action(async function(amount, options) {
    ...
    try {
      ...
      console.log(
        `${actualAmount.toString()} wei (${amountInEth} in eth) withdrawn from Ethereum Gateway.`
      );
      console.log(`Rinkeby tx hash: ${txHash}`);
    } catch (err) {
      console.error(err);
...
program
  ...
  .description(
    "attempt to complete a pending withdrawal via the Transfer Gateway"
  )
  .option("-g, --gas <number>", "Gas for the tx")
  .action(async function(options) {
    ...
    try {
      const extdev = loadExtdevAccount();
      const rinkeby = loadRinkebyAccount();
      console.log(rinkeby.account);
      client = extdev.client;

      const networkId = await rinkeby.web3js.eth.net.getId();
      const QuestCoinRinkebyAddress = Address.fromString(
        `eth:${QuestCoinRinkebyJSON.networks[networkId].address}`
      );
      const QuestLootRinkebyAddress = Address.fromString(
        `eth:${QuestLootRinkebyJSON.networks[networkId].address}`
      );
      const myRinkebyGatewayAddress = Address.fromString(
        `eth:${rinkebyGatewayAddress}`
      );
      const receipt = await getPendingWithdrawalReceipt(
        extdev.client,
        extdev.account
      );

      if (
        receipt.tokenContract.toString() === QuestCoinRinkebyAddress.toString()
      ) {
        console.log(
          `Found pending withdrawal of ${receipt.tokenAmount
            .div(coinMultiplier)
            .toString()} coins.`
        );
        const txHash = await withdrawCoinFromRinkebyGateway({
        ...
        console.log(
          `${receipt.tokenAmount
            .div(coinMultiplier)
            .toString()} tokens withdrawn from Etheruem Gateway.`
        );
        console.log(`Rinkeby tx hash: ${txHash}`);
      } else if (
        receipt.tokenContract.toString() === QuestLootRinkebyAddress.toString()
      ) {
        console.log(
          `Found pending withdrawal of token ${receipt.tokenId.toString()}.`
        );
        const txHash = await withdrawTokenFromRinkebyGateway({
          web3js: rinkeby.web3js,
          web3Account: rinkeby.account,
        ...
        console.log(
          `Token ${receipt.tokenId.toString()} withdrawn from Ethereum Gateway.`
        );
        console.log(`Rinkeby tx hash: ${txHash}`);
      } else if (
        receipt.tokenContract.toString() === myRinkebyGatewayAddress.toString()
      ) {
        console.log(
          `Found pending withdrawal of ${amountInEth.toString()} ETH.`
        );
        const txHash = await withdrawEthFromRinkebyGateway({
          web3js: rinkeby.web3js,
          web3Account: rinkeby.account,
        ...
        console.log(
          `${amountInWei.toString()} wei (${amountInEth.toString()} in eth) withdrawn from Etheruem Gateway.`
        );
        console.log(`Rinkeby tx hash: ${txHash}`);
      } else {
...
program
  ...
  .option(
    "-c, --chain <chain ID>",
    '"eth" for Rinkeby, "extdev" for PlasmaChain'
  )
...
program
  ...
  .action(async function(options) {
    try {
      ...
      console.log(
        `${ownerAddress} eth balance is ${balance} in wei (${balanceInEth} in eth)`
      );
    } catch (err) {
      console.error(err);
    }
...
program
  ...
  .action(async function(uid, options) {
    const { account, web3js } = loadRinkebyAccount();
    try {
      const tx = await depositTokenToGateway(
        web3js,
        uid,
        account.address,
        options.gas || 350000
      ...
      console.log(
        `Token ${uid} deposited, Rinkeby tx hash: ${tx.transactionHash}`
      );
    } catch (err) {
      console.error(err);
    }
...
program
  ...
  .action(async function(uid, options) {
    ...
    try {
      ...
      console.log(
        `Token ${uid} minted, Rinkeby tx hash: ${tx.transactionHash}`
      );
    } catch (err) {
      console.error(err);
    }
  ...
  .action(async function(options) {
    try {
      ...
      if (balance.tokens.length > 0) {
        console.log(
          `First ${balance.tokens.length} token(s): ${balance.tokens}`
        );
      }
    } catch (err) {
      console.error(err);
    ...
    try {
      ...
      if (contractType === "coin") {
        tokenRinkebyAddress =
          QuestCoinRinkebyJSON.networks[rinkebyNetworkId].address;
        rinkebyTxHash =
          QuestCoinRinkebyJSON.networks[rinkebyNetworkId].transactionHash;
        tokenExtdevAddress =
        ...
        console.log(
          'Specify which contracts you wish to map, "coin" or "token"'
        );
        return;
      }
      ...
      console.log(
        `Submitted request to map ${tokenExtdevAddress} to ${tokenRinkebyAddress}`
      );
    } catch (err) {
      console.error(err);
    } finally {
